from get_dataset import medical_qa_dataset_corpus, medical_qa_dataset_qeury
...
def run_bm25(corpus, queries, ground_truths, k=5):
    ...
    for i, q in enumerate(tqdm(queries, desc='running bm25 retrieval')):
        ...
        top_results = retriever.get_top_n(q.split(), corpus, k)
        formatted_results = [' '.join(i) for i in top_results] 
...
def main():
    ...
    args.corpus = medical_qa_dataset_corpus(args.corpus_path, tokenize=False)
    args.queries, args.ground_truths = medical_qa_dataset_qeury(args.query_path, tokenize=False)
