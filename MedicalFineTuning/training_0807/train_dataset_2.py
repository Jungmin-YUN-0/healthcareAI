...
def prompt_generate(example, system_prompt=None, use_system_prompt=False):
    ...
    # system/user 프롬프트를 고정된 의학 질문 형식으로 생성
    if use_system_prompt:
        prompt = f"[|system|]{system_prompt}\n[|user|]s{input_text}\n[|assistant|]{output_text}"
    else:
        prompt = f"[|system|]사용자의 의학적 질문에 답변해주세요.\n[|user|]{input_text}\n[|assistant|]{output_text}"
...
def main():
    ...
    train_dataset = dataset.shuffle(seed=args.random_seed)
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        "aaditya/Llama3-OpenBioLLM-8B" if model_name == "/home/jungmin/workspace/project/rapa/healthcareAI/ChatVector/ckpt/openbiollm_chatvector_KO" else model_name,
